ECOM/ECOM2 Interface
=================

OVERVIEW
--------

The `ECOM <https://www.cancapture.com/ecom>`_ is a low cost CAN interface made by EControls.
This interfaces is only supported on Windows using the ``ecom`` interface.


WINDOWS SUPPORT
---------------

Support though windows is achieved through a DLL very similar to the way the PCAN functions.  It uses a custom DLL supplied by
the manufacturer. 
...
---------------
    ...
    1. To install on Windows download the ECOM driver USB2CAN Windows device driver.  It is compatible with Win8/8.1, and Windows 10. (Written against driver version v1.0.2.1)    
    2. At the time of writing the standard install location 
    2. Copy the ecommlib.dll file from the install to same directory as 
